...
void setup()
{
  ...
  startup_animation();
  timeToUpdateDisplay = millis() + TIME_UPDATE_INTERVAL;
...
void loop()
{
  ...
  if (millis() > timeToChangeShow)
  {
    ...
    if (!temperature_first_show)
    {
      // temperature only once
      showMode = SHOW_HOUR_MIN;
    }
    if (showMode == SHOW_HOUR_MIN)
    {
      timeToChangeShow = millis() + SHOW_TIME_INTERVAL;
